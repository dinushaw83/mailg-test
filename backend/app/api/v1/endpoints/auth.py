...
class TokenRequest(BaseModel):
    ...
    User identification is required via one of: user_id, email, or name.
    Role is ALWAYS derived from the database (user.role) - the role field
    is ignored even if provided. This ensures the backend is the source
    of truth for permissions.
    """
    user_id: Optional[int] = Field(None, description="User ID (preferred lookup method)")
    email: Optional[str] = Field(None, description="User email (preferred lookup method)")
    name: Optional[str] = Field(None, description="User name (fallback, may be ambiguous)")
    role: Optional[str] = Field(None, description="DEPRECATED: Role is always derived from database. This field is ignored.")
...
def create_token(
    ...
    try:
        ...
        if token_request.user_id:
            user = lookup_db.query(User).filter(User.id == token_request.user_id).first()
            lookup_method = f"user_id={token_request.user_id}"
        elif token_request.email:
            ...
            lookup_method = f"email={normalized_email}"
        elif token_request.name:
            # Name lookup - may return multiple results, take first active non-deleted one
            users = lookup_db.query(User).filter(User.name == token_request.name).all()
            # Prefer non-deleted, active users
            user = next((u for u in users if not u.is_deleted and u.active), None)
            if not user and users:
                user = users[0]
            lookup_method = f"name={token_request.name}"
        else:
            raise HTTPException(
                status_code=400,
                detail="User identifier required. Provide email (preferred), user_id, or name.",
            )

        if not user:
            raise HTTPException(
                status_code=400,
                detail=f"User not found with {lookup_method}. Please check the identifier and try again.",
            )
        ...
        if user.suspended:
            raise HTTPException(
                status_code=401,
                detail="Cannot login: user account is suspended",
            )
        ...
        # Anti-spoofing: if the client asserts a role, it MUST match the DB role.
        # (We still derive the final role from the DB; this just fails fast on mismatch.)
        if token_request.role:
            requested_role = token_request.role.lower().strip()
            if requested_role and requested_role != token_role:
                raise HTTPException(
                    status_code=400,
                    detail=f"Role mismatch: requested '{requested_role}' but user is '{token_role}'",
                )
    finally:
