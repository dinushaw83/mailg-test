...
def authorized(allowed_roles: Optional[List[str]] = None):
    ...
    Usage:
        ...
        @router.put(
            ...
            dependencies=[Depends(authorized(["admin", "agent"]))],
        )
...
def is_staff(user: User) -> bool:
    """Check if user is staff (agent, case-insensitive).
    
    Args:
        user: User model instance.
        
    Returns:
        True if user role is 'agent'.
    """
    role = getattr(user, '_token_role', None) or user.role
    return role.lower() == "agent" if role else False


def is_end_user(user: User) -> bool:
    """Check if user is an end user (case-insensitive).
    
    Args:
        user: User model instance.
        
    Returns:
        True if user role is 'end-user'.
    """
    role = getattr(user, '_token_role', None) or user.role
    return role.lower() == "end-user" if role else False
