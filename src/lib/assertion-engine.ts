interface Assertion {
  ...
  export class AssertionEngine {
    async execute(assertion: Assertion, localStorage: Record<string, any>, modelResponse?: string): Promise<AssertionResult> {
      try {
        switch (assertion.operator) {
          ...
          case 'FACTUAL_VERIFICATION':
            return this.executeFactualVerification(assertion, localStorage, modelResponse)
          case 'REASONING_QUALITY':
            return this.executeReasoningQuality(assertion, localStorage, modelResponse)
          case 'INFORMATION_PRECISION':
              return this.executeInformationPrecision(assertion, localStorage, modelResponse)
    ...
    private getValue(path: string, data: any): any {
      ...
      // If path contains JSONPath expressions, use jsonpath-plus directly
      ...
      // Otherwise use the original simple path resolution
    ...
    private resolveDynamicDate(expression: string): string | null {
      const base = new Date()
    ...
    private async executeFactualVerification(
      assertion: Assertion,
      ...
      try {
        ...
        if (!assertionAny.description || !assertionAny.expected_facts) {
          return {
            actual: null,
            result: 'error',
        ...
        // Import the assertion operators to use FACTUAL_VERIFICATION
        ...
        const result = await assertionOperators.FACTUAL_VERIFICATION(actual, {
          description: assertionAny.description,
          expected_facts: assertionAny.expected_facts
        })
        ...
        assertionAny.expected_facts.forEach((expectedFact: any) => {
        ...
        const passThreshold = (assertionAny.pass_threshold_percent || 80) / 100 * 5 // Convert percentage to 1-5 scale
    ...
    async executeReasoningQuality(
      ...
      try {
        ...
        if (!assertionAny.description || !assertionAny.aspects) {
          return {
            actual: null,
            result: 'error',
    ...
    async executeInformationPrecision(
      assertion: Assertion,
      localStorage: Record<string, any>,
      modelResponse?: string
    ): Promise<AssertionResult> {
      try {
        ...
        if (!assertionAny.description || !assertionAny.expected_facts || !assertionAny.expected_reasonings) {
          return {
            actual: null,
            result: 'error',
        ...
        // Import the assertion operators to use INFORMATION_PRECISION
        const { assertionOperators } = await import('./utils/assertion-operators')
        ...
        const result = await assertionOperators.INFORMATION_PRECISION(actual, {
          description: assertionAny.description,
          expected_facts: assertionAny.expected_facts,
          expected_reasonings: assertionAny.expected_reasonings
        })
        
        // Calculate overall average from precision scores
        const scores = Object.values(result.precision_scores || {}) as number[]
        const averageScore = scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 0
        const passThreshold = (assertionAny.pass_threshold_percent || 75) / 100 * 5 // Convert percentage to 1-5 scale
    ...
    private executeFieldsUnchanged(assertion: Assertion, localStorage: Record<string, any>): AssertionResult {
      const actual = this.getValue(assertion.path!, localStorage)
      const expected = assertion.expected || assertion.expectation
      const options = assertion.options || {}
    ...
    private isNullishOrEmpty(value: any): boolean {
    ...
    private partialObjectMatch(actual: any, expected: any): boolean {
      // Check if all expected fields match in the actual object
      // Ignores extra fields in actual that are not in expected
      for (const key in expected) {
        if (expected.hasOwnProperty(key)) {
          const actualValue = actual[key];
          const expectedValue = expected[key];
          ...
          if (typeof expectedValue === 'object' && expectedValue !== null && 
              typeof actualValue === 'object' && actualValue !== null &&
              !Array.isArray(expectedValue)) {
            if (!this.partialObjectMatch(actualValue, expectedValue)) {
              return false;
            }
          } else {
    ...
    private filterNestedObject(actual: any, expected: any): any {
    ...
    private getNestedMismatches(actual: any, expected: any, parentKey: string): string[] {
