...
const DiffViewer = ({
  ...
  const expectedStr =
    expected !== undefined && expected !== null ? JSON.stringify(expected, null, 2) : "(not specified)";
  const actualStr = actual !== undefined && actual !== null ? JSON.stringify(actual, null, 2) : "(not specified)";
  ...
  return (
    <Box className={className} sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          {showLegend && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, fontSize: "0.75rem", color: "text.secondary" }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                <Box
                  sx={{ width: 12, height: 12, bgcolor: "#fee2e2", border: "1px solid #fecaca", borderRadius: "4px" }}
                />
                <span>Removed</span>
              </Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                <Box
                  sx={{ width: 12, height: 12, bgcolor: "#dcfce7", border: "1px solid #bbf7d0", borderRadius: "4px" }}
                />
                <span>Added</span>
              </Box>
            </Box>
      ...
      <Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 1, overflow: "hidden" }}>
        {hasDifferences ? (
          <Box sx={{ overflow: "auto", maxHeight: maxHeightValue }}>
            <EnhancedDiffView hunks={hunks} onExpandRange={() => {}} />
          </Box>
        ) : (
      ...
      <Box>
        ...
        >
          <IconButton size="small">{showRawJson ? <ExpandLess /> : <ExpandMore />}</IconButton>
          <Typography variant="body2" fontWeight="medium" color="text.secondary">
