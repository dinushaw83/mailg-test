...
export default function RemoveLabelModal({ open, onClose, onConfirm, multipleLabels = [] }) {
  const handleConfirm = () => {
    onConfirm();
    onClose();
  };
  ...
  if (multipleLabels.length > 1) {
        ...
        {multipleLabels.map((lbl, i) => (
          ...
          >
            ...
            {lbl.count > 0 && (
              <span className="ajO">
                {" "}
                ({lbl.count} {lbl.count === 1 ? "conversation" : "conversations"})
              </span>
            )}
          </Typography>
        ))}
    ...
    content = (
      <Typography sx={{ mb: 2, fontSize: 14 }}>
        {lbl.count ? (
          <>
            Remove the label "<strong>{lbl.fullPath}</strong>" from {lbl.count}{" "}
            {lbl.count === 1 ? "conversation" : "conversations"} and delete the label?
          </>
        ) : (
          <>
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ px: 3, mt: 2, pb: 1.5 }}>{content}</DialogContent>
