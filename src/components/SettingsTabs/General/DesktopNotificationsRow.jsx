...
import { SettingsRow, SettingsCell, BoldLabel, SubText, LearnMoreLink, SettingsRadio } from "./styles";
import { notificationManager, getNotificationPermission } from "../../../utils/notifications";
import { useNotificationContext } from "../../../contexts/NotificationContext";

export default function DesktopNotificationsRow({ localNotificationSettings, setLocalNotificationSettings }) {
  const { showDemoNotification } = useNotificationContext() || {};
  ...
  return (
    <SettingsRow>
      <SettingsCell side="left" width="20%">
        <BoldLabel style={{ marginLeft: "6px" }}>Desktop notifications:</BoldLabel>
        <br />
        <SubText>(allows MailG to display popup notifications on your desktop when new email messages arrive)</SubText>
        <br />
      ...
      <SettingsCell side="right">
        <label style={{ display: "block", marginBottom: "8px" }}>
          ...
          />
          <BoldLabel style={{ marginLeft: "6px" }}>New mail notifications on</BoldLabel> – Notify me when any new
          message arrives in my inbox or primary tab
        </label>
        <label style={{ display: "block", marginBottom: "8px" }}>
          <SettingsRadio
            type="radio"
          ...
          />
          <BoldLabel style={{ marginLeft: "6px" }}>Important mail notifications on</BoldLabel> – Notify me only when an
          important message arrives in my inbox
        </label>
        <label style={{ display: "block", marginBottom: "8px" }}>
          <SettingsRadio
            type="radio"
