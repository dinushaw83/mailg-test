 ...
 */
export const PLAIN_TEXT_FIELD_MAP: Record<string, string> = {
  body: "plain_body",
};

// Helper function inlined to avoid circular dependency
const stripHtmlTagsInline = (html: string): string => {
...
export const ASSERTION_OPERATORS: Record<
  string,
  (actual: any, expected: any, arrayKey?: string) => boolean
> = {
  ...
  ARRAY_CONTAINS: (actual, expected) => {
    ...
    return actual.some((item: any) => expected.includes(item));
  },
