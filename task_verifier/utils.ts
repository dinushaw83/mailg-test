...
export const evaluateFieldAssertions = (
  actualRow: any,
  fieldAssertions: FieldAssertion[]
): { allPassed: boolean; results: AssertionDetail[] } => {
  const results: AssertionDetail[] = [];
  let allPassed = true;
...
): { index: number; row: any; results: AssertionDetail[] } | null => {
 ...
 */
const hasOnlyTimestampChanges = (mod: any): boolean => {
...
export const compareResults = (
  ...
  const matchedActualRows: Record<string, Set<number>> = {};

  const markMatched = (tableName: string, type: string, idx: number) => {
    const key = `${tableName}:${type}`;
    if (!matchedActualRows[key]) matchedActualRows[key] = new Set();
    matchedActualRows[key].add(idx);
  };

  const isMatched = (tableName: string, type: string, idx: number): boolean => {
    const key = `${tableName}:${type}`;
    return matchedActualRows[key]?.has(idx) || false;
  };

  const shouldIgnoreTable = (tableName: string): boolean => {
    return ignoreTables.includes(tableName);
  };

  // STEP 1: Check all expected changes are present
  for (const [tableName, tableConfig] of Object.entries(tablesConfig) as any) {
    ...
    if (tableConfig.expect_modified) {
      for (const expectation of tableConfig.expect_modified) {
        const actualRows = actualTable.modified.map((m: any) => m.after || m);
        const found = findMatchingRow(
          actualRows,
          expectation.field_assertions,
    ...
    if (tableConfig.expect_no_deletes === true) {
      ...
      if (actualDeleted.length > 0) {
        countErrors.push({
          ...
          rows: actualDeleted.map((row: any) => ({
            id: row.id || row.primary_key?.id,
            summary: row.summary,
            name: row.name,
            title: row.title,
            rowType: "deleted" as const,
          })),
        });
      }
    }
    ...
    if (tableConfig.expect_exact_modified_count !== undefined) {
      ...
      const significantModifications = actualModified.filter(
        (mod: any) => !hasOnlyTimestampChanges(mod)
      );
      if (
        ...
        countErrors.push({
          ...
          rows: significantModifications.map((mod: any) => ({
    ...
    if (tableConfig.detect_content_duplicates && actualTable.added.length > 1) {
      ...
      actualTable.added.forEach((row: any) => {
  ...
  if (!allowUnexpectedChanges) {
    for (const [tableName, actualTable] of Object.entries(actualChanges) as [
      string,
      any,
    ][]) {
      // Skip ignored tables
